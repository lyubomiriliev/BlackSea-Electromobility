  ...
  return (
    <div className="w-full flex flex-col justify-center items-center">
      <div className="w-full py-10 mt-10 flex items-center bg-secondary">
        <h2 className="w-full text-2xl font-bold text-white text-center font-heading">
          {t("home.partners")}
        </h2>
      </div>
      <h2 className="text-xl font-medium mb-6"></h2>
      <div className="grid grid-cols-4 md:grid-cols-4 gap-4 px-5 md:px-0 mt-5">
        {muncipalities.map((muncipality, index) => (
          <Link to={muncipality.url} key={index}>
            <img
              src={muncipality.logo}
              alt="PartnerLogo"
              className="w-22 md:w-32 h-22 md:h-32"
            />
          </Link>
        ))}
      </div>
      <div className="w-full flex flex-col md:w-1/3 px-5 md:px-0">
